...
# TODO For Future Teams
Due to unforseen circumstances, there was miscommunication between how the URL is pointing towards the images that are stored on the google drive. Currently this application assumes that the link in the spreadsheet points directly to a google drive location (ie https://drive.google.com/drive/u/0/folders/xxxxx), however in production it is actually saved as a ("https://macro.script"). This means that the application can't open any images (Because it's looking for something different), and will therefore not actually run the models against anything. For future improvement this is where to focus on first. Please make sure to focus on this when following up and get those exact links that are in the production data to accurately go through the images, otherwise the url will need to be changed on the production data side to match what the application is looking for. One option for help would be to try to get ahold of the developer that wrote the previous codebase/website that our client is using as they might have more information being withheld from the client.
